...
class SettingsManager:
    ...
    setting_config_file = str(Path(__file__).parent.parent / "configs" / "settings.json")
    
    def __init__(self, main_window=None, config_file=setting_config_file):
        """
        初始化设置管理器
    ...
    def load_settings(self):
        """从配置文件加载设置"""
        try:
            # 配置文件应保存在项目根目录下的 configs/settings.json
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.logger.info(
                f"[{file_name}][{self.class_name}] 正在保存设置")
            return True
    ...
    def apply_system_settings(self):
        ...
        try:
            self.logger.info(
                f"[{file_name}][{self.class_name}] 正在应用系统设置...")
            ...
            if as_enable and not asm_enable:
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 正在配置开机自启动...")
                aso.configure_autostart(enable=True)
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 已经配置开机自启动...")
            else:
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 正在禁用开机自启动...")
                aso.configure_autostart(enable=False)
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 已经禁用开机自启动...")
            ...
            if self.main_window:
                ...
                if self.settings["minimize_to_tray"]:
                    self.logger.info(
                        f"[{file_name}][{self.class_name}] 正在开启最小化到后台运行...")
                    mtt.enable_running()
                    self.logger.info(
                        f"[{file_name}][{self.class_name}] 已经开启最小化到后台运行...")
                else:
                    self.logger.info(
                        f"[{file_name}][{self.class_name}] 正在关闭最小化到后台运行...")
                    mtt.disable_running()
                    self.logger.info(
                        f"[{file_name}][{self.class_name}] 已经关闭最小化到后台运行...")
            else:
                self.logger.warning(
                    f"[{file_name}][{self.class_name}][apply_system_settings] \
主窗口未设置，跳过最小化到托盘配置")
            ...
            try:
                ...
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 开启静默启动成功")
            
            except Exception as e:
                self.logger.error(
                    f"[{file_name}][{self.class_name}][apply_system_settings] 关闭静默启动失败\n错误: {e}")
                return False

            # 示例：处理通知设置
            if self.settings["desktop_notify"]:
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 启用桌面通知...")
                # nm.show_notification("启用桌面通知", "正在启用桌面通知")
                self.switch_system_notification(True)
                
            else:
                self.logger.info(
                    f"[{file_name}][{self.class_name}] 禁用桌面通知...")
                # nm.show_notification("禁用桌面通知", "已关闭桌面通知")
            
            self.logger.info(
                f"[{file_name}][{self.class_name}] 成功保存设置")
